...
async function generate_recentData(){
    const q = query(arref, orderBy("id", "desc"), limit(7));
...
//////////////TOP ARTICLE////////////////////
async function generate_topData(){
    const b = query(arref, orderBy("status","asc"), orderBy("id", "desc"),limit(2));
    let dataList = await onSnapshot(b, (snapshot) => {
        let data = []
        snapshot.docs.forEach((doc) => {
            data.push({...doc.data(), id: doc.id})
            document.getElementById("top_article_container").innerHTML += `
            <a class="top_article" href="articles/article.html?id=${doc.id}" id="top_article">
                <img src="${doc.data().thumbnail}" alt="">
                <h2 class="top_article_title">${doc.data().content[0].text}</h2>
            </a>
            `
        });
    })
...
//////////////MAIN ARTICLE////////////////////
async function generate_mainData(){
    const b = query(arref, orderBy("status","desc"), orderBy("id", "desc"),limit(1));
    let dataList = await onSnapshot(b, (snapshot) => {
        let data = []
        snapshot.docs.forEach((doc) => {
            data.push({...doc.data(), id: doc.id})
            document.getElementById("main_article_container").innerHTML += `
            <a class="main_article" href="articles/article.html?id=${doc.id}" >
                <div class="main_article_title_container">
                    <h1 class="main_article_title">${doc.data().content[0].text}</h1>
                </div>
                <img class="main_article_img" src="${doc.data().thumbnail}" alt="">
            </a>
            `
        });
    })
